...
def run():
    ...
    with grpc.insecure_channel('localhost:50051') as channel:
        ...
        logger.info(f"Server responded: {response.message}")
        ...
        def generate_messages():
            ...
            for msg in messages:
                logger.info(f"  - Sending: {msg.message}")
                yield msg
                
        response = stub.ClientStream(generate_messages())
        logger.info(f"Server response: {response.response} (Server ID: {response.server_id}, Sequence: {response.received_sequence}, Timestamp: {response.timestamp})")
        ...
        for response in response_stream:
            logger.info(f"  - Received: {response.response} (Server ID: {response.server_id}, Sequence: {response.received_sequence}, Timestamp: {response.timestamp})")
